...
export default function QumiWorkspace({ mode = 'tutor', circuit, results, onCircuitAction }) {
  ...
  return (
    <div className="flex flex-col h-full bg-[#0a0718] text-gray-100 rounded-3xl border border-purple-500/20 shadow-2xl overflow-hidden relative max-w-5xl mx-auto">
      ...
      {/* Input Area */}
      <div className="p-4 sm:p-6 shrink-0 relative bg-[#0a0718]">
        <div className="max-w-4xl mx-auto">
          
          {/* Always visible Contextual Chips */}
          {messages.length > 0 && (
            <div className="flex items-center gap-2 overflow-x-auto pb-3 hide-scrollbar">
              {contextualActions.map((action, idx) => (
                <button
                  ...
                  className="px-3 py-1.5 rounded-full bg-purple-500/10 border border-purple-500/20 hover:bg-purple-500/30 text-[11px] text-purple-200 transition-colors whitespace-nowrap"
                >
                  {action}
                </button>
              ))}
